...
class Driver(VISA_Driver):
    ...
    def performSetValue(self, quant, value, sweepRate=0.0, options={}):
        ...
        if quant.name in ('Bx', 'By', 'Bz'):
            ...
            while not (np.around(float(self.askAndLog('READ:DEV:' + dev + ':PSU:SIG:FLD').split(":",7)[6].replace('T','')),5) == np.around(float(self.askAndLog('READ:DEV:' + dev + ':PSU:SIG:FSET').split(":",7)[6].replace('T','')),5)):
                self.askAndLog('SET:DEV:' + dev + ':PSU:ACTN:RTOS')
            ...
            #tk.mainloop()
            print("waiting for switch heater")
        return value
